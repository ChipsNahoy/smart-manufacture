...
const char *data_topic = "raw_storage";
const char* mqtt_username = "RawStorage_esp";
const char* mqtt_password = "wololo";
const char *clientID = "RawStorage_Station_1";
...
void send_inventory_data(String id_value) {
  int size = sizeof(int) * 8 + 1;
  char temp[size * 5];
  char *ptr = temp;

  char id_char[id_value.length() + 1];
  id_value.toCharArray(id_char, id_value.length() + 1);

  strcpy(ptr, id_char);
  ptr += strlen(id_char);

  ptr += sprintf(ptr, ",%d", inventory1);
  ptr += sprintf(ptr, ",%d", inventory2);
  ptr += sprintf(ptr, ",%d", inventory3);
  ptr += sprintf(ptr, ",%d", inventory4);
  ptr += sprintf(ptr, ",%d", inventory5);

  client.publish("inventory", temp);
}
...
void loop()
{
  ...
  if (mfrc522.PICC_IsNewCardPresent() && mfrc522.PICC_ReadCardSerial()) {
      ...
      {
          if (byte(buffer_id[i]) != 0x00) id_val += (char)buffer_id[i];
      }
      id_val.trim();
      value += id_val;
      value += ",";
